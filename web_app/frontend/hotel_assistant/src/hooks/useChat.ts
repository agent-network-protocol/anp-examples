...
export interface RoomInfo {
  roomTypeId: string;
  ratePlanID: string; // 添加ratePlanID字段，这是预订所需的产品ID
  roomType: string;
  bedType: string;
  ...
  available: boolean;
  hotel: {
    hotelID: string; // Match backend field name (capital ID)
    hotelName: string;
    address: string;
    rating: number;
    price: string;
    ...
    try {
      ...
      const mockData: ChatResponse = {
        ...
        content: [
          {
            roomTypeId: "RT12345",
            ratePlanID: "RATE12345", // 添加ratePlanID字段
            roomType: "标准大床房",
            bedType: "大床",
            pricePerNight: 399,
          ...
          {
            roomTypeId: "RT12346",
            ratePlanID: "RATE12346", // 添加ratePlanID字段
            roomType: "舒适双床房",
            bedType: "双床",
            pricePerNight: 429,
